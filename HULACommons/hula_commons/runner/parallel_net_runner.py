...
class ParallelNetRunner(object):
    ...
    def _prepare_inputs(self):
        ...
        inputs, targets = self.__data_feeder.inputs(ConfigOptions.BATCH_SIZE.get_val(),
        ...
        if ConfigOptions.NUM_GPUS.get_val() > 1:
            ...
            inputs_list, targets_list = BaseFeeder.share_input(inputs,
                                                               targets,
    ...
    def _build_for_training(self):
        ...
        with self.__graph.as_default(), tf.device(consts.CPU_NAME+"0"):
            ...
            inputs_list, targets_list = self._prepare_inputs()
                ...
                for i in range(ConfigOptions.NUM_GPUS.get_val()):
                    with tf.device(consts.GPU_NAME + '%d' % i):
                        with tf.name_scope(consts.TOWER_NAME + '%d' % i):
                            ...
                            loss = self._tower_ops_training(inputs_list[i], targets_list[i])
